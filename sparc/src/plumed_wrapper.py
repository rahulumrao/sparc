...
from sparc.src.ase_md import NoseNVT, LangevinNVT, NPT, NVE, ExecuteMlpDynamics
...
def umbrella(
    ...
    SparcLog(f"Ensemble: {ensemble} | Thermostat: {thermostat_type} | Temperature: {temperature} K\n")
    ...
    for w, window in enumerate(umbrella_config['umbrella_windows']):
        ...
        if ensemble == 'NVE':
            ...
            dyn_dp = NPT(
                system=dp_atoms, timestep=timestep_fs, temperature=temperature,
                tau_t=tdamp, pressure=1.0, tau_p=1000, restart=False
            )
        else:  # NVT
            if thermostat_type == 'Nose':
                dyn_dp = NoseNVT(
                    atoms=dp_atoms, timestep=timestep_fs,
                    temperature=temperature, tdamp=tdamp, restart=False
                )
            else:
                dyn_dp = LangevinNVT(
                    atoms=dp_atoms, timestep=timestep_fs,
                    temperature=temperature, friction=friction, restart=False
                )
