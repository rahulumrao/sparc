...
def check_backend_mismatch(model_path: str, backend: str):
    ...
    if model_ext == '.pth' and backend != 'pytorch':
        ...
        SparcLog("  The model was trained with PyTorch but the current environment", level="ERROR")
        SparcLog("  has TensorFlow backend. To fix this either:", level="ERROR")
        SparcLog("    1. Install SPARC with DeePMD-GNN (PyTorch):  pip install deepmd-kit[torch]", level="ERROR")
        SparcLog("    2. Retrain models in the current TensorFlow environment", level="ERROR")
        SparcLog("=" * 80)
        raise RuntimeError(
        ...
        SparcLog(f"  Backend    : {backend.upper()} (installed)", level="ERROR")
        SparcLog("", level="ERROR")
        SparcLog("  The model was trained with TensorFlow but the current environment", level="ERROR")
        SparcLog("  has PyTorch backend. To fix this either:", level="ERROR")
        SparcLog("    1. Install SPARC with TensorFlow:  pip install deepmd-kit[tf]", level="ERROR")
        SparcLog("    2. Retrain models in the current PyTorch environment", level="ERROR")
        SparcLog("=" * 80)
        raise RuntimeError(
...
def log_md_setup(dyn, atoms, dir_name, write_dist=False):
    ...
    if is_npt:
        SparcLog(f"{step:<8} {epot:>11.4f} {ekin:>11.4f} {temp:>9.2f} {pressure_gpa:>9.4f} {volume:>9.2f}")
    else:
    ...
    if is_npt:
        with MDLogger(f"{dir_name}/AseMolDyn.log") as log:
            if step == 0:
                log.file.write(f"# {'Step':<6} {'Epot':<10} {'Ekin':<10} {'Total':<10} "
                                f"{'Temp':<6} {'Pressure(GPa)':<14} {'Volume(Å³)':<12}\n")
            log.file.write(f"{float(step):<8} {epot:<10.6f} {ekin:<10.6f} {total:<10.6f} "
                            f"{temp:<6.2f} {pressure_gpa:<14.6f} {volume:<12.4f}\n")
    else:
        with MDLogger(f"{dir_name}/AseMolDyn.log") as log:
            if step == 0:
                log.file.write(f"# {'Step':<6} {'Epot':<10} {'Ekin':<10} {'Total':<10} {'Temp':<6}\n")
    ...
    try:
        ...
        if 'state' in progress_data and 'iteration' in progress_data:
            ...
            if i_candidate is None:
                ...
                prev_candidate_dir = Path(f"iter_{prev_iter:06d}") / "02.dpmd" / "dft_candidates"
                ...
                if prev_candidate_dir.exists():
                    ...
                    if candidates_traj.exists():
                        ...
                        SparcLog("-" * 80)
                        SparcLog(f" Detected restart from ML/MD stage (02.dpmd)")
                        SparcLog(f" Found {num_candidates} candidates from iteration {prev_iter}")
                        SparcLog(f" Setting idx = {i_candidate}, candidate = {n_candidate}")
                        SparcLog("-" * 80)
...
def get_initial_structure(
    ...
    if config.mlip_setup.restart_exploration and iter > 0:
        ...
        prev_traj_file = parent_dir / f"iter_{prev_iter:06d}" / "02.dpmd" / config.output.dptraj_file
        ...
        if os.path.exists(prev_traj_file):
            try:
                if restart_mode == "last":
                    ...
                    SparcLog(f" Restart exploration: Using last frame from iteration {prev_iter}")
                    ...
                    SparcLog(f" Restart exploration: Run {sample_idx+1} using frame {random_idx}/{len(prev_traj)} from iteration {prev_iter}")
                
                elif restart_mode == "candidates":
                    # Each run starts from a DIFFERENT random DFT candidate (safest)
                    candidate_file = parent_dir / f"iter_{prev_iter:06d}" / "00.dft" / config.output.aimdtraj_file
                    ...
                    if os.path.exists(candidate_file):
                        ...
                        random.seed(iter * 1000 + sample_idx)
                        random_candidates = random.sample(range(len(candidates)), min(len(candidates), 100))
                        cand_idx = random_candidates[sample_idx % len(random_candidates)]
                        
                        atoms = candidates[cand_idx]
                        SparcLog(f" Restart exploration: Run {sample_idx+1} using DFT candidate {cand_idx+1}/{len(candidates)} from iteration {prev_iter}")
                    else:
                        SparcLog(f" Candidates file not found, using last frame instead", level="WARNING")
                        atoms = read(prev_traj_file, index=-1)
                
                else:
                    raise ValueError(f"Invalid restart_mode: {restart_mode}. Options: 'last', 'random', 'candidates'")
                
                return atoms
            
            except Exception as e:
                SparcLog(f" Warning: Could not load from previous iteration: {e}", level="WARNING")
                SparcLog(f" Falling back to original structure")
        
        else:
            SparcLog(f" Warning: Previous trajectory not found at {prev_traj_file}", level="WARNING")
            SparcLog(f" Falling back to original structure")
