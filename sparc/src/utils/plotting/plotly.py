...
def _plot_energy_parity(e_true, e_pred, e_unit):
    ...
    fig.add_trace(go.Scatter(
        x=e_true, y=e_pred,
        mode='markers',
        marker=dict(size=8, color='blue', opacity=0.7, line=dict(color='black', width=1)),
        name='Data',
...
def _plot_forces_parity(f_true, f_pred):
    ...
    for i, comp in enumerate(components):
        ...
        fig.add_trace(go.Scatter(
            x=f_t, y=f_p,
            mode='markers',
            marker=dict(size=6, color='blue', opacity=0.6, line=dict(color='black', width=0.5)),
            showlegend=False,
    ...
    fig.update_layout(width=1400, height=400, template='plotly_white', hovermode='closest')
    return fig
...
def _plot_all_parity(e_true, e_pred, e_unit, f_true, f_pred):
    ...
    fig.update_layout(width=1000, height=800, template='plotly_white', hovermode='closest')
    return fig
...
def PlotForceDeviation(root_dir=".", iteration_window="all", target_iteration=None, 
                       dmin=0.05, dmax=0.5, save_fig=None):
    """
    ...
    # Add threshold lines
    max_steps = max([max(steps) for _, data in data_dict.items() for _, steps, _ in data])
    
    fig.add_hline(y=dmin, line_dash="dash", line_color="black", line_width=2, 
                  opacity=0.6, annotation_text=f"dmin={dmin}")
    fig.add_hline(y=dmax, line_dash="dash", line_color="black", line_width=2,
                  opacity=0.6, annotation_text=f"dmax={dmax}")
...
def PlotPotentialEnergy(root_dir=".", iteration_window="all", target_iteration=None,
                        traj_filename="AseMD.traj", save_fig=None):
    """
...
def PlotDistribution(root_dir=".", iteration_window="all", target_iteration=None,
                     traj_filename="AseMD.traj", bins=50, save_fig=None):
    """
    ...
    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=("Energy Distribution", "Max Force Distribution")
    )
...
def PlotPES(root_dir=".", iteration_window="all", target_iteration=None,
            traj_filename="AseMD.traj", coord_type="distance",
            atom_indices=None, save_fig=None):
    """
    ...
    fig.add_trace(go.Scatter(
        ...
        y=coord2_array,
        mode='markers',
        marker=dict(size=4, color='white', opacity=0.5, line=dict(color='black', width=0.5)),
        name='Sampled points',
...
def PlotTemp(root_dir=".", iteration_window="all", target_iteration=None,
             traj_filename="AseMD.traj", save_fig=None):
    """
