...
def PlotForceDeviation(
    ...
    if target_iteration is not None:
        selected_dirs = [
            d for d in iter_dirs if int(d.split("_")[-1]) == target_iteration
        ]
        ...
        selected_dirs = [
            d
            for d in iter_dirs
            if iteration_window[0] <= int(d.split("_")[-1]) <= iteration_window[1]
        ]
...
def PlotPotentialEnergy(
    ...
    for iter_dir in selected_dirs:
        ...
        energies = [
            float(np.asarray(item.get_potential_energy()).flat[0]) for item in traj
        ]
...
def ReadColvar(file_path="COLVAR"):
    ...
    return pd.read_csv(file_path, sep=r"\s+", comment="#", names=column_names)
