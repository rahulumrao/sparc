...
def main():
    ...
    subparsers = parser.add_subparsers(
        dest="command", required=True, help="Available analysis commands"
    )
    ...
    cmp_parser = subparsers.add_parser(
        "get_energies",
        help="Calculate DFT and ML potential energies for frames in a trajectory",
    )
    cmp_parser.add_argument(
        "--dft_file",
        type=str,
        required=True,
        help="Path to the reference trajectory file containing DFT energies (e.g., OUTCAR).",
    )
    cmp_parser.add_argument(
        "--ifmt",
        type=str,
        default="vasp-out",
        help="ASE-compatible format for reading the input trajectory [default: vasp-out]",
    )
    cmp_parser.add_argument(
        "--skip",
        type=int,
        default=1,
        help="Stride for frame extraction: analyze every nth frame [default: 1]",
    )
    cmp_parser.add_argument(
        "--model",
        type=str,
    ...
    if args.command == "get_energies":
        ...
        SparcLog(
            f"Unknown analysis command: {args.command}",
            level="ERROR",
            origin="ANALYSIS",
        )
        sys.exit(1)
