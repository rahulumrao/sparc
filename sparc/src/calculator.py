...
class SetupDFTCalculator:
    ...
    def xtb(self):
        ...
        if not template_file or not Path(template_file).exists():
            raise CalculatorError(
                "xTB template_file required in dft_calculator config"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        engine = config['dft_calculator'].get('engine') or \
                config['dft_calculator'].get('name')
