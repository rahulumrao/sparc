...
`SPARC` automates this process by integrating the PLUMED library [@tribello2014plumed] with AL protocol into a single modular workflow (see Figure 1). This allows the user to use any advanced sampling technique implemented in PLUMED to explore the configurational space and finding reactive configurations enabling scalable and reproducible MLIPs for generalized chemical environment. Other tools such as `janus-core`[@kasoar2024januscore], `ArcaNN`[@david2025arcann], and `FLARE`[@vandermause2020flare] also supports MLIP driven simulations with enhanced sampling. However, the integration of active learning with the PLUMED library into a single workflow remains an active area of development.
...
ML/MD output is stored in ASE trajectory formats as this enables the broader ecosystem of analysis tools that already support ASE compatible formats. Since `SPARC` manages all stages via ASE, the workflow requires a Python environment with necessary dependencies. This makes the workflow highly portable across computing environments and suitable for both exploratory studies and large-scale production runs.
