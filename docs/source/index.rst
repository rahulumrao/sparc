...
This package is designed to work seamlessly with VASP, DeepMD-kit, and PLUMED, providing users with powerful tools 
for efficient simulation and model improvement.

Scientific Overview
--------------------
SPARC employs an active learning workflow to iteratively refine machine learning potentials using first-principles calculations. 
...
- **First-principles calculations** with `VASP <vasp_>`_ for data generation
- **Machine learning potential training** with `DeepMD-kit <deepmd_>`_ package
- **Machine learning molecular dynamics (MLMD)** simulations for efficient and scalable modeling
- **Active learning** for continuous model improvement through iterative training and data selection
- **Potential Energy Surface Exploration** through integration with `PLUMED <plumed_>`_
